...
In Tabella \ref{tab:aws_services}, infine, viene mostrata la tassonomia, secondo le classi appena definite, dei servizi usati nell'implementazione del sistema di rilevazione dei DPI, con una breve descrizione del loro funzionamento, reperibile dettagliamente dalla documentazione Amazon\cite{b11AWSDoc}. 
...
%note ulteriori su batching e streaming. 
%Un buon modo per comprendere l'elaborazione in streaming è confrontarla con l'elaborazione batch. Flink, infatti, supporta entrambi i tipi di elaborazione, stream e batch. Puoi scrivere il codice una sola volta e, a seconda del contesto in cui viene eseguito, questo può funzionare come un job di streaming o come un job batch. Per Flink, il batch non è altro che un caso speciale del runtime. L'effetto di questa distinzione è che consente una serie di ottimizzazioni. Queste ottimizzazioni per la modalità batch sono possibili solo con stream delimitati, mentre gli stream non delimitati richiedono l'elaborazione in streaming.

%Ci si aspetta che un processore di stream produca risultati rapidamente, con una latenza minima end-to-end per l'intera pipeline. Perché ciò avvenga, l'intera pipeline deve rimanere in esecuzione in modo continuo. Al contrario, quando Flink opera in modalità batch, può ridurre i requisiti di risorse eseguendo le fasi della pipeline in sequenza anziché in parallelo. Se pensiamo all'esempio precedente, in cui il primo passo consisteva nel filtrare gli eventi "arancioni", in modalità batch potremmo prima rimuovere tutti quegli eventi arancioni dal set di dati e poi elaborare il set filtrato attraverso il passo successivo della pipeline, e così via.

%Una delle ottimizzazioni disponibili per l'elaborazione batch consiste nel preordinare i dati in ingresso per facilitarne l'elaborazione. Ad esempio, i database talvolta implementano i join ordinando prima le tabelle di input, rendendo il join più semplice. Flink può fare lo stesso, ma solo in modalità batch. Quando Flink opera in modalità streaming, deve accettare ogni evento appena arriva. Talvolta Flink non è in grado di elaborare immediatamente un evento e potrebbe doverlo memorizzare temporaneamente fino a quando non arrivano i dati necessari. Questi buffer devono essere archiviati in un sistema di stato durevole, in modo che il loro contenuto non venga perso se il task manager fallisce e deve essere riavviato.

%In modalità batch, l'elaborazione può continuare fino al completamento del job, momento in cui possono essere prodotti i risultati finali. In modalità streaming, ogni evento potrebbe essere l'ultimo, quindi i risultati vengono prodotti in modo incrementale dopo ogni evento, oppure possono essere prodotti periodicamente basandosi su timer. In modalità batch, Flink può recuperare dai fallimenti riavviando il job dall'inizio. In modalità streaming, invece, Flink cerca di minimizzare i tempi di inattività riprendendo da uno snapshot recente.

%Più avanti nel corso spiegherò come funziona questo meccanismo in modo più dettagliato. Flink offre garanzie di esecuzione esattamente-once sia in modalità batch che streaming, ma ottenere questo risultato per lo streaming è piuttosto complesso, poiché i riavvii devono avvenire rapidamente e senza una rielaborazione significativa. Come abbiamo visto, l'elaborazione in streaming pone alcune richieste interessanti al runtime e, per certi versi, è più complessa da supportare rispetto all'elaborazione batch.

%Vale la pena affrontare questa complessità extra? Assolutamente sì. Alcuni casi d'uso sono possibili solo con un processore di stream. Ad esempio, in casi d'uso in cui la bassa latenza è un requisito critico, come il monitoraggio dei data center o il rilevamento delle frodi, la latenza dell'elaborazione batch potrebbe renderla impraticabile. Quindi, alcuni casi d'uso necessitano di un processore di stream. Ma ci sono casi d'uso che richiedono un processore batch? Non proprio. L'elaborazione in streaming è più potente dell'elaborazione batch, e qualsiasi caso d'uso soddisfatto dall'elaborazione batch potrebbe essere gestito da un processore di stream. Tuttavia, è utile avere un runtime che supporti esplicitamente sia l'elaborazione batch che quella streaming, poiché l'elaborazione batch può essere molto più efficiente. 

%definisci cosa sono le sorgenti, cosa sta in mezzo (applicazione flink) e il sink si scende nel mondo dello streaming: qui dovresti definire le api, tra cui datastream, gli operatori e il windowing
...
\begin{itemize}
	...
	\item \textbf{repartition}: si tratta di un'operazione di filtro e raggruppamento. Questo tipo di operazione è più costosa del forward. I dati deve essere serializzati, perché in base a qual è l'operatore di destinazione, potrebbe essere inviato nella rete. Un altro operatore infatti potrebbe trovarsi su un nodo diverso da quello di partenza.
...
In Figura \ref{fig:dataflow-l} viene mostrato un esempio di dataflow logico, con diverse trasformazioni. Il source può essere di diverse tipologie, e nell'ambito di questa tesi viene utilizzato il Kinesis Data Stream connector, per estrarre i record dal servizio gestito Amazon. %fornisci un esempio e aggiungi un commento per le trasformazioni che salvano internamente lo stato.
...
Un altro dettaglio, che non viene descritto nella rappresentazione del dataflow fisico, è il salvataggio dello stato. Ciascun operatore stateful, come FlatMapFunction (o join?), possono infatti persistere localmente informazioni associate a più elementi processati. %espandi questa parte, mostrando che viene implementato proprio a questo livello il concetto di snapshot. 
...
Idealmente, si vorrebbe salvare lo stato in maniera indefinita, ma questo comporta dei problemi in termini di risorse. Il flusso di dati, come già visto, è indefinito, quindi l'unica soluzione per poter mantenere uno stato di dimensioni accettabili è quello di effettuare le elaborazioni per un numero di eventi limitato. L'utilizzo delle finestre serve esattamente per questo tipo di scenario. Nell'implementazione del sistema sono state usate due tipologie di finestre: quelle basate sulla conta (countwindow) e quelle su timer (window temporali). %espandi questa parte per concludere
