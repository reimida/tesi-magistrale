...
In letteratura, sono stati individuati due approcci che rientrano nel dominio di questa ricerca. Nel complesso, dimostrano delle differenze nella modalità di implementazione del sistema, rispetto a quanto realizzato in questa tesi, ma aiutano a fornire contesto alla trattazione. Il primo metodo {cita} è totalmente integrato con il cloud, ma si serve del provider Microsoft Azure. L'obiettivo del paper è quello di analizzare direttamente su una telecamera IoT, in real time, l'input proveniente da una Learning Factory, un ambiente che simula quello di un impianto manufatturiero. Si tratta di un luogo pensato a scopo di training e di ricerca, ma non per questo esente da rischi. Il modello eseguito sul SoC del dispositivo è stato allenato e customizzato per essere lanciato su dell'hardware con una potenza di calcolo limitata. La fase di generazione del modello avviene sul cloud, successivamente, una versione leggera viene caricata sul dispositivo. Come nel sistema implementato in questa tesi, l'architettura è di tipo edge-cloud. Le differenze sostanziali consistono nella locazione del modello e nella modalità i cui esso viene acceduto. Non c'è bisogno infatti di invocazioni a servizi nel cloud una volta che l'algoritmo di machine learning è stato deployato sull'edge, come invece succede nel sistema proposto. Inoltre nell'implementazione proposta viene chiamata una API per un modello già specializzato nell'analisi dei dispositivi di sicurezza, come si vedrà nel capitolo successivo. Quindi difficilmente una customizzazione del modello offerto da Amazon può ottenere delle prestazioni migliori. Nel related work, inoltre, l'obiettivo è l'identificazione degli occhiali protettivi, mentre il modello utilizzato in questa soluzione è in grado di identificare ulteriori classi di dispositivi di sicurezza. Il modello deployato sull'edge raggiunge l'obiettivo di inferenza in tempo reale, ma le prestazioni sono comunque limitate, indipendentemente dal tipo di allenamento eseguito. Sono stati infatti seguiti due approcci: nel primo caso il training è effettuato su immagini presenti in rete, il che non si adattava allo scenario della Learning Factory. Nel secondo invece, sono state utilizzate immagini provenienti dal sito, ottenendo un benchmark migliore, seppur limitato.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/relw1.png}
    \caption{Prestazioni con immagini della Learning Factory}
    \label{fig:relw1}
\end{figure}

Il modello rileva tre classi principali: occhiali protettivi, volti e persone. Gli ultimi due tag sono importanti per capire se i dispositivi sono indossati, mentre il primo è importante per l'identificazione dell'oggetto di interesse. I risultati confermano la difficoltà generale dei modelli nel trovare oggetti piccoli all'interno di un frame. Sul totale dei casi in cui il modello è in grado di identificare il dispositivo di sicurezza, solo la metà sono occhiali protettivi. In generale l'algoritmo riesce ad identificare pochissime volte i dpi, proprio per le loro piccole dimensioni.

Il secondo approccio analizzato {cita}, propone una soluzione completamente on premise. Si tratta di un meccanismo molto più sofisticato rispetto all'oggetto di questo scritto, ma, trattandosi di un lavoro recente e con prestazioni ottimali, permette di ottenere una panoramica migliore sugli sviluppi in questo campo. Il sistema proposto non si limita solo a rilevare i dispositivi di sicurezza, ma anche gli incidenti e il controllo delle macchine. In base a ciò che il modello utilizzato riesce ad identificare, viene generata una matrice composta dalla probabilità che un evento dannoso possa verificarsi e dal tipo di danno che può essere generato. Il sistema, al matching sulla matrice deve rispondere opportunamente. Questo meccanismo introduce un livello di granularità nella reazione agli eventi, in quanto, ad esempio non ha senso spegnere un macchinario se un lavoratore non ha un dipositivo di sicurezza indossato, ma allo stesso non si trova in un'area pericolosa. 
